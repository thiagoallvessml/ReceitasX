...
c = c.replace(
    ...
    "sb.from('produtos').select('*, receitas(custo_ing, custo_emb, custo_eqp, unidades)').eq('user_id', user.id).order('nome')"
);
...
c = c.replace(
    ...
    `produtos = (resProd.data || []).map(p => {
                ...
                if (p.receitas) {
                    const sum = parseFloat(p.receitas.custo_ing || 0) + parseFloat(p.receitas.custo_emb || 0) + parseFloat(p.receitas.custo_eqp || 0);
