...
# Target the actual script block which starts with <script>\n  /* ... DADOS
match = re.search(r'<script>\s*/\*.*?DADOS[\s\S]*?</script>', html)
if match:
    html = html[:match.start()] + new_script + html[match.end():]
else:
    # fallback to just searching for let despesas
    match2 = re.search(r'<script>[\s\S]*?let despesas = \[\];[\s\S]*?</script>', html)
    if match2:
        html = html[:match2.start()] + new_script + html[match2.end():]
